...
* Mining pool design
  ...
  The mining pool of DGE also adopts the elastic supply mechanism, that is, the supply of mining pool is proportional to the amount of lock-in of liquidity. That is to say, each flow input will generate 50% of the flow into ore and put it into the mining pool. However, However, when each amount of fluidity is reduced, the corresponding amount of ore is correspondingly reduced from the ore pool. If there is no transaction, it remains unchanged.
