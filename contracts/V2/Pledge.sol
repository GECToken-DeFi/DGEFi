...
Authority {
    ...
    mapping(address =>mapping(string =>uint256)) public tokenpledge;
    //DGE数量
    mapping(address =>mapping(string =>uint256)) public tokenTDGEpledgeDGEAmount;
    //质押率,单位为 10000倍，即 8000= 8000/10000=80%,大写
    mapping(string =>uint256) public tokenPledgeRate;
    //质押币种，所有地址总量
    mapping(string =>uint256) public tokenPledgeTotalAmount;
    //质押后，新生成DGE，打入用户地址，但为锁定状态
    mapping(address =>uint256) public tokenPledgeGetDGELockedAmount;
    //初始化代币参数
    constructor() public {}
    ///
    ...
    // 获得质押率
    function getTokenPledgeRate(string memory _tokenName) public view returns(uint256) {
        return tokenPledgeRate[_tokenName];
    }
    ////	// 获得质押币种，所有地址总量
    function getTokenPledgeTotalAmount(string memory _tokenName) public view returns(uint256) {
        return tokenPledgeTotalAmount[_tokenName];
    }
    // 获得质押后，新生成DGE，打入用户地址，但为锁定状态
    function getTokenPledgeGetDGELockedAmount(address _from) public view onlyGovenors1 returns(uint256) {
        return tokenPledgeGetDGELockedAmount[_from];
    }
    // 获得质押的DGE数量  tokenTDGEpledgeDGEAmount
    function getTokenTDGEpledgeDGEAmount(address _from, string memory _tokenName) public view returns(uint256) {
        return tokenTDGEpledgeDGEAmount[_from][_tokenName];
    }
    // 获得质押的token  tokenpledge
    function getTokenpledge(address _from, string memory _tokenName) public view returns(uint256) {
    ...
    function doPledge(address _userAddress, string memory tokenName, uint256 _tokenAmount, uint256 _sameDGEAmount) public onlyGovenors1 returns(bool success) {
    ...
    //质押定向生成锁定的DGE
    function doPledgeMintDGE(address _userAddress, string memory tokenName, uint256 _sameDGEAmount) internal onlyGovenors1 returns(bool success) {
    ...
    function withdrawPledge(address _userAddress, string memory tokenName, uint256 _tokenAmount) public onlyGovenors1 returns(bool success) {
    ...
    //解除质押定向销毁锁定的DGE
    function withdrawPledgeBurnDGE(address _userAddress, string memory tokenName, uint256 _tokenAmount) internal onlyGovenors1 returns(bool success) {
