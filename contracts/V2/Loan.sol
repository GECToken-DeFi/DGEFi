...
Authority {
    ...
    LoanItem[] public loans;
    mapping(address =>LoanItem[]) public loanUserDGEAmount;
    //此地址此币种用户借款条目数量
    mapping(address =>uint256) public LoanItemTimes;
    //借款抵押品质押率,单位为 10000倍，即 8000= 8000/10000=80%,大写
    mapping(string =>uint256) public laonGuaranteeRate;
    //贷款产品，统一利率手续费 //uint256 loanTime; //unit s, 3个月等//uint256 loanPayRate; //用户付出的贷款利率
    mapping(uint256 =>uint256) public productRate;
    //初始化代币参数
    constructor() public {}
    ///
    //设置借款抵押品质押率
    ...
    //以及贷款产品，统一利率手续费 
    function setLaonProductRate(uint256 _loanTime, uint256 _loanPayRate) public onlyGovenors1 returns(bool success) {
        productRate[_loanTime] = _loanPayRate;
        return true;
    }
    // 获得借款抵押率
    function getLaonGuaranteeRate(string memory _guaranteeName) public view returns(uint256) {
        return laonGuaranteeRate[_guaranteeName];
    }
    // 贷款产品，统一利率手续费
    function getProductRate(uint256 _loanTime) public view returns(uint256) {
        return productRate[_loanTime];
    }
    //写入此次用户贷款的数据
    function writeToLoanItem(address _userAddress, string memory _guaranteeDec, uint256 _getDGEAmount, uint256 _loanTime) public onlyGovenors1 returns(bool success) {
    ...
    function payBackUpdateLoanItem(address _userAddress, uint256 _loanProductId) public onlyGovenors1 returns(bool success) {
    ...
    // 显示用户贷款数据
    function showAddressLoanItem(address _userAddress) public view returns(LoanItem[] memory) {
        LoanItem[] storage loa = loanUserDGEAmount[_userAddress];
        return loa;
    }
}